...
// type RecordValue models.RecordValue
...
func main() {
	...
	fmt.Println("bootstrap.servers: %s", conf["bootstrap.servers"])
	...
	for run == true {
		...
		case sig := <-sigchan:
			...
			run = false
		default:
			recordKey := "go-ex-" + gofakeit.HexUint16() + strconv.Itoa(int(gofakeit.Date().Unix()))
			data := &User{
				Name: gofakeit.Name(),
